    ...
    public LiveData<Resource<Event<String>>> requestEcho(String text) {
        ...
        try {
            ...
            HttpRequest httpRequest = new HttpRequest.Builder(url).setMethod(
                HttpRequest.METHOD_POST).setTimeout(5000).setFormUrlEncodedBody(data).build();
            httpClient.enqueue(httpRequest, new HttpResponse.Callback() {
                ...
                public void onResponse(HttpRequest httpRequest, HttpResponse httpResponse) {
                    if (httpResponse.getCode() == HttpURLConnection.HTTP_OK) {
                        result.postValue(Resource.success(new Event<>(
                            new String(httpResponse.getBody(), StandardCharsets.UTF_8).trim())));
                    } else {
