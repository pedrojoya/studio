...
public class TextViewUtils {
    ...
    public interface OnImeActionListener {
        void onActionSearch(TextView v, KeyEvent event);
    }

    public static void setOnImeActionListener(@NonNull TextView textView, int action,
            @NonNull OnImeActionListener listener) {
        textView.setOnEditorActionListener((v, actionId, event) -> {
            if (actionId == action) {
                listener.onActionSearch(v, event);
                return true;
            }
            return false;
        });
