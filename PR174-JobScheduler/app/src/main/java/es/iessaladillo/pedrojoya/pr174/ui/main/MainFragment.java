...
import es.iessaladillo.pedrojoya.pr174.base.EventObserver;
import es.iessaladillo.pedrojoya.pr174.services.ExportToTextFileService;

@SuppressWarnings("WeakerAccess")
public class MainFragment extends Fragment {

    private static final int EXPORT_JOB_ID = 1;
    ...
    public void export() {
        ...
        if (currentStudents != null) {
            JobScheduler jobScheduler = (JobScheduler) requireActivity().getSystemService(
                Context.JOB_SCHEDULER_SERVICE);
            if (jobScheduler != null) {
                jobScheduler.schedule(
                    ExportToTextFileService.newJobInfo(requireContext(), EXPORT_JOB_ID,
                        currentStudents));
            }
        }
    }
