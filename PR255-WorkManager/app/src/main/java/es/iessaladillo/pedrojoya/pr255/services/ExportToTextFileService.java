...
import es.iessaladillo.pedrojoya.pr255.utils.DataUtils;
...
public class ExportToTextFileService extends Worker {
    ...
    public static OneTimeWorkRequest.Builder newOneTimeWorkRequestBuilder(List<String> students) {
        Data inputData = new Data.Builder().putStringArray(EXTRA_INPUT_STRING_ARRAY,
            students.toArray(new String[0])).build();
        return new OneTimeWorkRequest.Builder(ExportToTextFileService.class).setInputData(
            inputData);
    }
    ...
    private void showResultNotification(File outputFile) {
        Intent intent = new Intent(Intent.ACTION_VIEW).setDataAndType(
            FileProvider.getUriForFile(getApplicationContext(),
                BuildConfig.APPLICATION_ID + ".fileprovider", outputFile), "text/plain").addFlags(
    ...
    public Result doWork() {
        List<String> students = Arrays.asList(
            DataUtils.requireStringArray(getInputData(), EXTRA_INPUT_STRING_ARRAY));
        ...
        try {
            ...
            return Result.failure(
                new Data.Builder().putString(EXTRA_RESULT_URI_STRING, e.getMessage()).build());
        }
