...
public class MainActivity extends AppCompatActivity {
    ...
    private void requestData() {
        Intent intent = Henson.with(this).gotoStudentActivity().student(mStudent).build();
        startActivityForResult(intent, RC_STUDENT);
    }

    @Override
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (resultCode == RESULT_OK && requestCode == RC_STUDENT) {
            mStudent = StudentActivity.getStudentFromExtra(data);
            showStudent();
        }
    }
